...
export function GET(request: Request) {
  return withPrivateApi(request, async ({ client, ownerId }) => {
    const { data, error } = await client
      ...
      .from("documents")
      .select(
        "id,name,source_mime,availability,created_at,document_versions(download_status,evidence_version_id,created_at),ingestion_items(status,stage,sanitized_error,started_at,finished_at)"
      )
      .eq("owner_id", ownerId)
