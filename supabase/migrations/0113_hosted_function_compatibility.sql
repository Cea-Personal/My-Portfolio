-- Keep security-definer publication hashing able to resolve the pgcrypto extension
-- installed in the hosted project's public schema.
alter function app.stage_portfolio_publication()
  set search_path = pg_catalog, app, published, public;

-- PostgreSQL versions used by Supabase do not expose jsonb_object_length(). Count
-- object keys instead while retaining the same bounded-property contract.
...
begin
  ...
  if jsonb_typeof(coalesce(requested_properties, '{}'::jsonb)) <> 'object'
    or (select count(*) from jsonb_object_keys(coalesce(requested_properties, '{}'::jsonb))) > 12 then
